...
def sample_sd(model,
    ...
    for i, t in enumerate(tqdm(timesteps)):
        ...
        # TODO: Why not simply unsqueeze(0) instead of view? is it related to views > 2?
    ...
    images = model.vae.decode(latents/model.vae.config.scaling_factor,
                                return_dict=False,
...
def sample_match_sd(model,
    ...
    width = model.unet.config.sample_size * model.vae_scale_factor # 64 * 8 for SD 1.5
    batch_size = 1      # TODO: Support larger batch sizes, maybe    
    ...
    if scheduler:
        ...
        scheduler = Scheduler.from_pretrained("runwayml/stable-diffusion-v1-5", subfolder="scheduler")
    ...
    # prepare latent variables
    num_channels_latents = model.unet.config.in_channels # 4 for SD1.5
    latents = model.prepare_latents(
            batch_size * num_images_per_prompt,
            num_channels_latents,
            height,
    ...
    latents = latents.repeat(2, 1, 1, 1) # 2 x 4 x 64 x 64
    ...
    for i, t in enumerate(tqdm(timesteps)):
        ...
        noise_pred = torch.cat([noise_pred_path_1.unsqueeze(0), noise_pred_path_2.unsqueeze(0)], dim=0)
               
        output = model.scheduler.step(noise_pred, 
                                    t, 
                                    latents, 
                                    return_dict=True)
